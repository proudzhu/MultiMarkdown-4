	...
	switch (n->key) {
		...
		case METADATA:
			if (scratch->extensions & EXT_SNIPPET)
				break;
			mmd_g_string_append_printf(out, "{\\info\n");
		...
		case METAKEY:
			...
			if (strcmp(n->str, "title") == 0) {
				mmd_g_string_append_printf(out, "{\\title ");
				print_rtf_node(out, n->children, scratch);
				mmd_g_string_append_printf(out, "}\n");
			} else if (strcmp(n->str, "author") == 0) {
				mmd_g_string_append_printf(out, "{\\author ");
				print_rtf_node(out, n->children, scratch);
				mmd_g_string_append_printf(out, "}\n");
			} else if (strcmp(n->str, "affiliation") == 0) {
				mmd_g_string_append_printf(out, "{\\company ");
				print_rtf_node(out, n->children, scratch);
				mmd_g_string_append_printf(out, "}\n");
				...
				mmd_g_string_append_printf(out, "{\\keywords ");
				print_rtf_node(out, n->children, scratch);
				mmd_g_string_append_printf(out, "}\n");
			} else if (strcmp(n->str, "copyright") == 0) {
				mmd_g_string_append_printf(out, "{\\*\\copyright ");
				print_rtf_node(out, n->children, scratch);
				mmd_g_string_append_printf(out, "}\n");
			} else if (strcmp(n->str, "comment") == 0) {
				mmd_g_string_append_printf(out, "{\\doccomm ");
				print_rtf_node(out, n->children, scratch);
				mmd_g_string_append_printf(out, "}\n");
			} else if (strcmp(n->str, "subject") == 0) {
				mmd_g_string_append_printf(out, "{\\subject ");
				print_rtf_node(out, n->children, scratch);
				mmd_g_string_append_printf(out, "}\n");
			}
			break;
		...
		case VERBATIMFENCE:
			pad_rtf(out, 2, scratch);
			mmd_g_string_append_printf(out, "{\\pard " kCodeStyle);
...
//			print_rtf_string(out, n->str, scratch);
		...
		case PARA:
			...
			switch (scratch->odf_para_type) {
				case BLOCKQUOTE:
					mmd_g_string_append_printf(out, "{\\pard " kQuoteStyle);
					break;
				case NOTEREFERENCE:
				case CITATION:
					mmd_g_string_append_printf(out, "{\\pard " kNoteStyle);
					break;
				case CODE:
				case VERBATIM:
				case VERBATIMFENCE:
					...
					break;
				default:
					mmd_g_string_append_printf(out, "{\\pard " kNormalStyle);
					break;
			}
			print_rtf_node_tree(out,n->children,scratch);
			mmd_g_string_append_printf(out, "\n\\par}\n");
		...
		case H1: case H2: case H3: case H4: case H5: case H6:
			...
			switch (lev) {
				case 1:
					mmd_g_string_append_printf(out, "{\\pard " kH1);
					break;
				case 2:
					mmd_g_string_append_printf(out, "{\\pard " kH2);
					break;
				case 3:
					mmd_g_string_append_printf(out, "{\\pard " kH3);
					break;
				case 4:
					mmd_g_string_append_printf(out, "{\\pard " kH4);
					break;
				case 5:
					mmd_g_string_append_printf(out, "{\\pard " kH5);
					break;
				case 6:
					mmd_g_string_append_printf(out, "{\\pard " kH6);
					break;
			}
			if (n->children->key == AUTOLABEL) {
				temp = label_from_string(n->children->str);
				mmd_g_string_append_printf(out, "{\\*\\bkmkstart %s}{\\*\\bkmkend %s}",temp, temp);
			...
			free(temp);
			mmd_g_string_append_printf(out, "\\par}\n");
			scratch->padded = 1;
			break;
		case TABLE:
			...
			print_rtf_node_tree(out, n->children, scratch);
			if ((n->children != NULL) && (n->children->key == TABLECAPTION)) {
				mmd_g_string_append_printf(out, "{\\pard " kNormalStyle "\\qc ");
				print_rtf_node_tree(out, n->children->children, scratch);
				mmd_g_string_append_printf(out, "\\par}\n");
			}
			mmd_g_string_append_printf(out, "\\pard\\par\n");
			scratch->padded = 1;
			break;
		case TABLELABEL:
		case TABLECAPTION:
		...
		case TABLEROW:
			...
			mmd_g_string_append_printf(out, "\\trowd\\trautofit1\n");
			for (i=0; i < strlen(scratch->table_alignment); i++) {
				mmd_g_string_append_printf(out, "\\cellx%d\n",i+1);
			}
			print_rtf_node_tree(out, n->children, scratch);
			mmd_g_string_append_printf(out, "\\row\n");
			break;
		case TABLECELL:
			...
			mmd_g_string_append_printf(out, "\\intbl");

			if (scratch->cell_type == 'h') {
				mmd_g_string_append_printf(out, "\\qc{\\b ");
			} else {
				if ( strncmp(&temp[lev],"r",1) == 0) {
					mmd_g_string_append_printf(out, "\\qr");
				} else if ( strncmp(&temp[lev],"R",1) == 0) {
					mmd_g_string_append_printf(out, "\\qr");
				} else if ( strncmp(&temp[lev],"c",1) == 0) {
					mmd_g_string_append_printf(out, "\\qc");
				} else if ( strncmp(&temp[lev],"C",1) == 0) {
					mmd_g_string_append_printf(out, "\\qc");
				} else {
					mmd_g_string_append_printf(out, "\\ql");
				}
			}
			mmd_g_string_append_printf(out, " {");
			print_rtf_node_tree(out, n->children, scratch);
			...
			mmd_g_string_append_printf(out, "}\\cell\n");
			scratch->table_column++;
			break;
		case STRONG:
			mmd_g_string_append_printf(out, "{\\b ");
			print_rtf_node_tree(out,n->children,scratch);
			mmd_g_string_append_printf(out, "}");
			break;
		case EMPH:
			mmd_g_string_append_printf(out, "{\\i ");
			print_rtf_node_tree(out,n->children,scratch);
			mmd_g_string_append_printf(out, "}");
			break;
		case LINEBREAK:
			mmd_g_string_append_printf(out, "\\line ");
			break;
		case LINK:
			...
			/* Insert link */
			mmd_g_string_append_printf(out, "{\\field{\\*\\fldinst{HYPERLINK \"");
			print_rtf_string(out, temp_link_data->source, scratch);
			mmd_g_string_append_printf(out, "\"}}{\\fldrslt ");
			if (n->children != NULL)
				print_rtf_node_tree(out, n->children, scratch);
			mmd_g_string_append_printf(out, "}}");
		...
		case BULLETLIST:
			pad(out, 2, scratch);
			mmd_g_string_append_printf(out, "\\ls1\\ilvl0 ");
			scratch->padded = 0;
			print_rtf_node_tree(out, n->children, scratch);
			break;
		case ORDEREDLIST:
			...
			break;
		case LISTITEM:
			mmd_g_string_append_printf(out, "{\\listtext \\'95 }");
			print_rtf_node_tree(out, n->children, scratch);
			break;
		case NOTEREFERENCE:
			...
			mmd_g_string_append_printf(out, "{\\super\\chftn}{\\footnote\\pard\\plain\\chtfn ");
			...
			if ((n->link_data != NULL) && (strncmp(n->link_data->label,"[#",2) == 0)) {
				...
				if (lev != 0) {
					...
					if (lev > scratch->max_footnote_num) {
						...
						mmd_g_string_append_printf(out, "{\\super\\chftn}{\\footnote\\ftnalt\\pard\\plain\\chtfn ");
						scratch->padded = 2;
						...
						mmd_g_string_append_printf(out, "REUSE CITATION");
					}
				} else {
		...
		case IMAGEBLOCK:
		case IMAGE:
			mmd_g_string_append_printf(out, "IMAGES CANNOT BE INSERTED INTO AN RTF DOCUMENT FROM MULTIMARKDOWN \\\n");
			break;
		case VARIABLE:
...
void begin_rtf_output(MMD_GString *out, node* list, scratch_pad *scratch) {
	mmd_g_string_append_printf(out, "{\\rtf1\\ansi\\deff0 {\\fonttbl\n" \
		...
		case LSQUOTE:
			switch (scratch->language) {
				...
				case GERMANGUILL:
					...
					break;
				default:
					mmd_g_string_append_printf(out,"\\'91");
				}
			break;
		case RSQUOTE:
			switch (scratch->language) {
				...
				case GERMANGUILL:
					...
					break;
				default:
					mmd_g_string_append_printf(out,"\\'92");
				}
			break;
		case APOS:
			mmd_g_string_append_printf(out,"\\'27");
			break;
		case LDQUOTE:
			switch (scratch->language) {
				case DUTCH:
				...
				case SWEDISH:
					...
					break;
				default:
					mmd_g_string_append_printf(out,"\\'93");
				}
			break;
		case RDQUOTE:
			switch (scratch->language) {
				...
				case FRENCH:
					...
					break;
				default:
					mmd_g_string_append_printf(out,"\\'94");
				}
			break;
		case NDASH:
			mmd_g_string_append_printf(out,"\\'96");
			break;
		case MDASH:
			mmd_g_string_append_printf(out,"\\'97");
			break;
		case ELLIP:
			mmd_g_string_append_printf(out,"\\'85");
			break;
			default:;
	}
}
...
void pad_rtf(MMD_GString *out, int num, scratch_pad *scratch) {
