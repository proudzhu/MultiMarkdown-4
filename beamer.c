...
void print_beamer_node(MMD_GString *out, node *n, scratch_pad *scratch) {
	...
	switch (n->key) {
		case FOOTER:
			...
			mmd_g_string_append_printf(out, "\\mode<all>\n");
			...
			mmd_g_string_append_printf(out, "\\mode*\n");
			break;
		case LISTITEM:
			pad(out, 1, scratch);
		...
		case HEADINGSECTION:
			if (n->children->key -H1 + scratch->baseheaderlevel == 3) {
				...
				pad(out, 1, scratch);
				mmd_g_string_append_printf(out, "\\mode<article>{\n");
				scratch->padded = 0;
		...
		case H1: case H2: case H3: case H4: case H5: case H6:
			...
			switch (lev) {
				...
				case 2:
					...
					break;
				case 3:
					mmd_g_string_append_printf(out, "\\frametitle{");
					break;
				default:
...
void print_beamer_endnotes(MMD_GString *out, scratch_pad *scratch) {
	...
	pad(out, 2, scratch);
	mmd_g_string_append_printf(out, "\\part{Bibliography}\n\\begin{frame}[allowframebreaks]\n\\frametitle{Bibliography}\n\\def\\newblock{}\n\\begin{thebibliography}{0}\n");
	while ( note != NULL) {
