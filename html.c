...
void print_col_group(MMD_GString *out,scratch_pad *scratch);
...
void print_html_node(MMD_GString *out, node *n, scratch_pad *scratch) {
	...
	if ((scratch->extensions & EXT_COMPLETE)
		...
		!((n->key == FOOTER) || (n->key == METADATA))) {
			mmd_g_string_append_printf(out, "</head>\n<body>\n");
	...
	switch (n->key) {
		...
		case ABBR:
			if (strlen(n->children->str) == 0) {
				mmd_g_string_append_printf(out, "<abbr>");
			} else {
				mmd_g_string_append_printf(out, "<abbr title=\"");
		...
		case PARA:
			pad(out, 2, scratch);
			mmd_g_string_append_printf(out, "<p>");
			print_html_node_tree(out,n->children,scratch);
			if (scratch->footnote_to_print != 0){
				...
				if (scratch->footnote_para_counter == 0) {
					...
					mmd_g_string_append_printf(out, " <a href=\"#fnref:%d\" title=\"return to article\" class=\"reversefootnote\">&#160;&#8617;</a>", random);
		...
		case HRULE:
			pad(out, 2, scratch);
			mmd_g_string_append_printf(out, "<hr />");
			scratch->padded = 0;
			break;
		case HTMLBLOCK:
			pad(out, 2, scratch);
		...
		case VERBATIMFENCE:
			...
			if ((n->children != NULL) && (n->children->key == VERBATIMTYPE)) {
				...
				if (strlen(n->children->str) > 0)
					mmd_g_string_append_printf(out, "<pre><code class=\"%s\">", n->children->str);
				else
					mmd_g_string_append_printf(out, "%s", "<pre><code>");
			} else {
				mmd_g_string_append_printf(out, "%s", "<pre><code>");
			}
			print_html_string(out, n->str, scratch);
			mmd_g_string_append_printf(out, "%s", "</code></pre>");
			scratch->padded = 0;
			break;
		case BULLETLIST:
			pad(out, 2, scratch);
			mmd_g_string_append_printf(out, "%s", "<ul>");
			scratch->padded = 0;
			print_html_node_tree(out, n->children, scratch);
			pad(out, 1, scratch);
			mmd_g_string_append_printf(out, "%s", "</ul>");
		...
		case ORDEREDLIST:
			pad(out, 2, scratch);
			mmd_g_string_append_printf(out, "%s", "<ol>");
			scratch->padded = 0;
			print_html_node_tree(out, n->children, scratch);
			pad(out, 1, scratch);
			mmd_g_string_append_printf(out, "</ol>");
			scratch->padded = 0;
			break;
		case LISTITEM:
			pad(out, 1, scratch);
			mmd_g_string_append_printf(out, "<li>");
			scratch->padded = 2;
			print_html_node_tree(out, n->children, scratch);
			mmd_g_string_append_printf(out, "</li>");
			scratch->padded = 0;
		...
		case METAKEY:
			...
			if (strcmp(n->str, "title") == 0) {
				mmd_g_string_append_printf(out, "\t<title>");
				print_html_node(out, n->children, scratch);
				mmd_g_string_append_printf(out, "</title>\n");
			} else if (strcmp(n->str, "css") == 0) {
				mmd_g_string_append_printf(out, "\t<link type=\"text/css\" rel=\"stylesheet\" href=\"");
				print_html_node(out, n->children, scratch);
				mmd_g_string_append_printf(out, "\"/>\n");
				...
				mmd_g_string_append_printf(out,"\t<meta name=\"%s\" content=\"",n->str);
				print_html_node(out,n->children,scratch);
				mmd_g_string_append_printf(out,"\"/>\n");
			}
			break;
		...
		case DOUBLEQUOTED:
			...
			break;
		case LINEBREAK:
			mmd_g_string_append_printf(out, "<br/>\n");
			break;
		case MATHSPAN:
			temp = strdup(n->str);
			if (temp[0] == '$') {
				...
				if (temp[1] == '$') {
					temp[strlen(temp)-1] = '\0';
					mmd_g_string_append_printf(out, "<span class=\"math\">\\[%s\\]</span>",&temp[2]);
				} else {
					mmd_g_string_append_printf(out, "<span class=\"math\">\\(%s\\)</span>",&temp[1]);
				}
			} else if (temp[strlen(temp) - 1] == ']') {
				temp[strlen(temp) - 3] = '\0';
				mmd_g_string_append_printf(out, "<span class=\"math\">%s\\]</span>",temp);
			} else {
				temp[strlen(temp) - 3] = '\0';
				mmd_g_string_append_printf(out, "<span class=\"math\">%s\\)</span>",temp);
			}
			...
			break;
		case STRONG:
			mmd_g_string_append_printf(out, "<strong>");
			print_html_node_tree(out,n->children,scratch);
			mmd_g_string_append_printf(out, "</strong>");
		break;
		case EMPH:
			mmd_g_string_append_printf(out, "<em>");
			print_html_node_tree(out,n->children,scratch);
			mmd_g_string_append_printf(out, "</em>");
		break;
		case LINKREFERENCE:
	...
	fprintf(stderr, "print html link: '%s'\n",n->str);
			...
			mmd_g_string_append_printf(out, "<a");
			if (n->link_data->source != NULL) {
				mmd_g_string_append_printf(out, " href=\"");
	...
	fprintf(stderr, "print image\n");
			...
			if (n->key == IMAGEBLOCK)
				mmd_g_string_append_printf(out, "<figure>\n");
			/* Do we have proper info? */
			if ((n->link_data->label == NULL) &&
			(n->link_data->source == NULL)) {
				/* we seem to be a [foo][] style link */
...
#endif
			mmd_g_string_append_printf(out, "<img");
			if (n->link_data->source != NULL)
				mmd_g_string_append_printf(out, " src=\"%s\"",n->link_data->source);
			if (n->children != NULL) {
				mmd_g_string_append_printf(out, " alt=\"");
				temp_str = mmd_g_string_new("");
				print_raw_node_tree(temp_str, n->children);
				...
				mmd_g_string_append_printf(out, " alt=\"%s\"",n->link_data->title);
			}
			if (!(scratch->extensions & EXT_COMPATIBILITY)) {
				if (n->link_data->label != NULL) {
					temp = label_from_string(n->link_data->label);
					mmd_g_string_append_printf(out, " id=\"%s\"",temp);
					free(temp);
				}
			}
			if ((n->link_data->title != NULL) && (strlen(n->link_data->title) > 0)) {
	...
	#endif
				mmd_g_string_append_printf(out, " style=\"");
				if (height != NULL)
					mmd_g_string_append_printf(out, "height:%s;", height);
				if (width != NULL)
					mmd_g_string_append_printf(out, "width:%s;", width);
				mmd_g_string_append_printf(out, "\"");
			}
	#ifdef DEBUG_ON
			...
			if (n->key == IMAGEBLOCK) {
				if (n->children != NULL) {
					...
					if (temp_str->currentStringLength > 0) {
						...
						mmd_g_string_append_printf(out, "</figcaption>");
					}
					mmd_g_string_free(temp_str, true);
				}
				mmd_g_string_append_printf(out,"\n</figure>");
		...
		case NOTEREFERENCE:
			...
			if (temp_node->key == GLOSSARYSOURCE) {
				if (lev > scratch->max_footnote_num) {
					mmd_g_string_append_printf(out, "<a href=\"#fn:%d\" id=\"fnref:%d\" title=\"see footnote\" class=\"footnote glossary\">[%d]</a>",
						random, random, lev);
						scratch->max_footnote_num = lev;
				} else {
					mmd_g_string_append_printf(out, "<a href=\"#fn:%d\" title=\"see footnote\" class=\"footnote glossary\">[%d]</a>",
						random, lev);
				}
			} else {
				if (lev > scratch->max_footnote_num) {
					mmd_g_string_append_printf(out, "<a href=\"#fn:%d\" id=\"fnref:%d\" title=\"see footnote\" class=\"footnote\">[%d]</a>",
						random, random, lev);
						scratch->max_footnote_num = lev;
				} else {
					mmd_g_string_append_printf(out, "<a href=\"#fn:%d\" title=\"see footnote\" class=\"footnote\">[%d]</a>",
						random, lev);
				}
			}
			break;
	...
	fprintf(stderr, "print cite\n");
			...
			if ((n->link_data != NULL) && (strncmp(n->link_data->label,"[#",2) == 0)) {
				...
				if (n->key == NOCITATION) {
					mmd_g_string_append_printf(out, "<span class=\"notcited\" id=\"%s\"/>",n->str);
				} else {
					mmd_g_string_append_printf(out, "<span class=\"externalcitation\">");
					mmd_g_string_append_printf(out, "</span>");
				}
			} else {
#ifdef DEBUG_ON
				...
				if (lev != 0) {
					...
					if (n->key == NOCITATION) {
						mmd_g_string_append_printf(out, "<span class=\"notcited\" id=\"%d\">",random);
					} else {
						mmd_g_string_append_printf(out, "<a class=\"citation\" href=\"#fn:%d\" title=\"Jump to citation\">[",
							random);
							if (n->children != NULL) {
								mmd_g_string_append_printf(out, "<span class=\"locator\">");
								print_html_node(out, n->children, scratch);
								mmd_g_string_append_printf(out, "</span>, %d]", lev);
							} else {
								mmd_g_string_append_printf(out, "%d]", lev);
							}
					}
					mmd_g_string_append_printf(out, "<span class=\"citekey\" style=\"display:none\">%s</span>", n->link_data->label);
					if (n->key == NOCITATION) {
						mmd_g_string_append_printf(out, "</span>");
					} else {
						mmd_g_string_append_printf(out, "</a>");
				...
				fprintf(stderr, "no match for cite: '%s'\n",n->link_data->label);
					...
					if ((n->link_data != NULL) && (n->key == NOCITATION)) {
						mmd_g_string_append_printf(out, "<span class=\"notcited\" id=\"%s\"/>",n->link_data->label);
						...
						mmd_g_string_append_printf(out, "#%s]</span>",n->link_data->label);
					}
				}
			}
#ifdef DEBUG_ON
		...
		case VARIABLE:
			...
			break;
		case GLOSSARYTERM:
			mmd_g_string_append_printf(out,"<span class=\"glossary name\">");
			print_html_string(out, n->children->str, scratch);
			mmd_g_string_append_printf(out, "</span>");
			if ((n->next != NULL) && (n->next->key == GLOSSARYSORTKEY) ) {
				mmd_g_string_append_printf(out, "<span class=\"glossary sort\" style=\"display:none\">");
		...
		case GLOSSARYSORTKEY:
			break;
		case CODE:
			mmd_g_string_append_printf(out, "<code>");
			print_html_string(out, n->str, scratch);
			mmd_g_string_append_printf(out, "</code>");
			break;
		...
		case BLOCKQUOTE:
			pad(out,2, scratch);
			mmd_g_string_append_printf(out, "<blockquote>\n");
			scratch->padded = 2;
			print_html_node_tree(out, n->children, scratch);
			pad(out,1, scratch);
			mmd_g_string_append_printf(out, "</blockquote>");
			...
			break;
		case RAW:
			mmd_g_string_append_printf(out, "RAW:");
			mmd_g_string_append_printf(out,"%s",n->str);
			break;
		case HTML:
			mmd_g_string_append_printf(out, "%s", n->str);
		...
		case DEFLIST:
			pad(out,2, scratch);
			scratch->padded = 1;
			mmd_g_string_append_printf(out, "<dl>\n");
			print_html_node_tree(out, n->children, scratch);
			mmd_g_string_append_printf(out, "</dl>");
			scratch->padded = 0;
			break;
		case TERM:
			pad(out,1, scratch);
			mmd_g_string_append_printf(out, "<dt>");
			print_html_node_tree(out, n->children, scratch);
			mmd_g_string_append_printf(out, "</dt>\n");
			scratch->padded = 1;
			break;
		case DEFINITION:
			pad(out,1, scratch);
			scratch->padded = 1;
			mmd_g_string_append_printf(out, "<dd>");
			print_html_node_tree(out, n->children, scratch);
			mmd_g_string_append_printf(out, "</dd>\n");
			scratch->padded = 0;
			break;
		case TABLE:
			pad(out,2, scratch);
			mmd_g_string_append_printf(out, "<table>\n");
			print_html_node_tree(out, n->children, scratch);
			mmd_g_string_append_printf(out, "</table>\n");
			scratch->cell_type = 0;
			scratch->padded = 1;
		...
		case TABLECAPTION:
			...
			mmd_g_string_append_printf(out, "<caption id=\"%s\">", temp);
			print_html_node_tree(out, n->children, scratch);
			mmd_g_string_append_printf(out, "</caption>\n");
			free(temp);
			break;
		case TABLELABEL:
			break;
		case TABLEHEAD:
			...
			mmd_g_string_append_printf(out, "\n<thead>\n");
			print_html_node_tree(out, n->children, scratch);
			mmd_g_string_append_printf(out, "</thead>\n");
			scratch->cell_type = 'd';
			break;
		case TABLEBODY:
			mmd_g_string_append_printf(out, "\n<tbody>\n");
			print_html_node_tree(out, n->children, scratch);
			mmd_g_string_append_printf(out, "</tbody>\n");
			break;
		case TABLEROW:
			mmd_g_string_append_printf(out, "<tr>\n");
		...
		case TABLECELL:
			...
			if ( strncmp(&temp[lev],"r",1) == 0) {
				mmd_g_string_append_printf(out, "\t<t%c style=\"text-align:right;\"", temp_type);
			} else if ( strncmp(&temp[lev],"R",1) == 0) {
				mmd_g_string_append_printf(out, "\t<t%c style=\"text-align:right;\"", temp_type);
			} else if ( strncmp(&temp[lev],"c",1) == 0) {
				mmd_g_string_append_printf(out, "\t<t%c style=\"text-align:center;\"", temp_type);
			} else if ( strncmp(&temp[lev],"C",1) == 0) {
				mmd_g_string_append_printf(out, "\t<t%c style=\"text-align:center;\"", temp_type);
			} else {
				mmd_g_string_append_printf(out, "\t<t%c style=\"text-align:left;\"", temp_type);
			}
			if ((n->children != NULL) && (n->children->key == CELLSPAN)) {
				mmd_g_string_append_printf(out, " colspan=\"%d\"",(int)strlen(n->children->str)+1);
				scratch->table_column += (int)strlen(n->children->str);
			}
			mmd_g_string_append_printf(out, ">");
			scratch->padded = 2;
			print_html_node_tree(out, n->children, scratch);
			mmd_g_string_append_printf(out, "</t%c>\n", temp_type);
			scratch->table_column++;
			break;
		case CELLSPAN:
			break;
		...
		case GLOSSARYLABEL:
			break;
		case SUPERSCRIPT:
			mmd_g_string_append_printf(out, "<sup>%s</sup>",n->str);
			break;
		case SUBSCRIPT:
			mmd_g_string_append_printf(out, "<sub>%s</sub>",n->str);
			break;
		case ABBREVIATION:
		case KEY_COUNTER:
			break;
		case TOC:
			mmd_g_string_append_printf(out, "<div class=\"TOC\">\n");
			print_html_node_tree(out,n->children, scratch);
			mmd_g_string_append_printf(out, "\n</div>");
			break;
		default:
...
void print_html_endnotes(MMD_GString *out, scratch_pad *scratch) {
	...
	pad(out,2, scratch);
	mmd_g_string_append_printf(out, "<div class=\"footnotes\">\n<hr />\n<ol>");
	while ( note != NULL) {
		...
		if (note->key == CITATIONSOURCE) {
			mmd_g_string_append_printf(out, "<li id=\"fn:%d\" class=\"citation\"><span class=\"citekey\" style=\"display:none\">%s</span>",
				random, note->str);
		} else {
			mmd_g_string_append_printf(out, "<li id=\"fn:%d\">\n", random);
		}
		...
		switch (*str) {
			...
			default:
				if ((scratch->obfuscate == true) && (extension(EXT_OBFUSCATE, scratch->extensions))
					&& ((int) *str == (((int) *str) & 127))) {
						if (ran_num_next() % 2 == 0)
						mmd_g_string_append_printf(out, "&#%d;", (int) *str);
					else
						mmd_g_string_append_printf(out, "&#x%x;", (unsigned int) *str);
				} else {
...
void print_col_group(MMD_GString *out,scratch_pad *scratch) {
	char *temp;
	int lev;
	mmd_g_string_append_printf(out, "<colgroup>\n");
	temp = scratch->table_alignment;
	for (lev=0;lev<strlen(temp);lev++) {
		if ( strncmp(&temp[lev],"r",1) == 0) {
			mmd_g_string_append_printf(out, "<col style=\"text-align:right;\"/>\n");
		} else if ( strncmp(&temp[lev],"R",1) == 0) {
			mmd_g_string_append_printf(out, "<col style=\"text-align:right;\" class=\"extended\"/>\n");
		} else if ( strncmp(&temp[lev],"c",1) == 0) {
			mmd_g_string_append_printf(out, "<col style=\"text-align:center;\"/>\n");
		} else if ( strncmp(&temp[lev],"C",1) == 0) {
			mmd_g_string_append_printf(out, "<col style=\"text-align:center;\" class=\"extended\"/>\n");
		} else if ( strncmp(&temp[lev],"L",1) == 0) {
			mmd_g_string_append_printf(out, "<col style=\"text-align:left;\" class=\"extended\"/>\n");
		} else {
			mmd_g_string_append_printf(out, "<col style=\"text-align:left;\"/>\n");
		}
	}
	mmd_g_string_append_printf(out, "</colgroup>\n");
}
