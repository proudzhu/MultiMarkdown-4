...
MMD_GString *heading_name[7];
MMD_GString *used_abbreviations;
...
void dump_tree(node* n, scratch_pad *scratch)
{
	...
	while(n != NULL){
       ...
       mmd_g_string_append(scratch->lyx_debug_pad,"  "); /* add a level */
	   ...
	   scratch->lyx_debug_nest--;
	   mmd_g_string_free(scratch->lyx_debug_pad,TRUE); /* don' see a way to shorten the string */
	   scratch->lyx_debug_pad = mmd_g_string_new(""); /* so create a new, shorter one */
	   for(i=0;i<scratch->lyx_debug_nest;i++){
	    mmd_g_string_append(scratch->lyx_debug_pad,"  ");
       }
...
void perform_lyx_output(MMD_GString *out, node* list, scratch_pad *scratch)
{
	...
	heading_name[0] = mmd_g_string_new("Part");
	heading_name[1] = mmd_g_string_new("Chapter");
	heading_name[2] = mmd_g_string_new("Section");
	heading_name[3] = mmd_g_string_new("Subsection");
	heading_name[4] = mmd_g_string_new("Subsubsection");
	heading_name[5] = mmd_g_string_new("Paragraph");
	heading_name[6] = mmd_g_string_new("Subparagraph");

		/* get user supplied heading section names and base header level these both
		   affect creating the prefixes */

	MMD_GString *lyx_headings = mmd_g_string_new("");
	used_abbreviations = mmd_g_string_new("");
	int hcount;
	hcount = 0;
	const char s[2] = ",";
    char *token;
	...
	if (tree_contains_key(list, METAKEY)) {
		...
		if (headings != NULL) {
			...
			mmd_g_string_append(lyx_headings,key);
			token = strtok(lyx_headings->str, s);
			 while( token != NULL )  {
			   mmd_g_string_free(heading_name[hcount],TRUE);
			   cleaned = clean_string(token);
			   heading_name[hcount] = mmd_g_string_new(cleaned);
			   free(cleaned);
               token = strtok(NULL, s);
               hcount++;
	...
	mmd_g_string_free(lyx_headings,TRUE);
	...
	if (isbeamer){
		mmd_g_string_free(heading_name[1],TRUE);
		heading_name[1] = mmd_g_string_new("Section");
		mmd_g_string_free(heading_name[2],TRUE);
		heading_name[2] = mmd_g_string_new("Frame");
	...
	int i;
	for (i=0;i<=6;i++){
		mmd_g_string_free(heading_name[i],TRUE);
	}
    mmd_g_string_free(used_abbreviations,TRUE);
}
...
bool begin_lyx_output(MMD_GString *out, node* list, scratch_pad *scratch) {
	...
	mmd_g_string_append(out, "#LyX File created by multimarkdown\n");
	mmd_g_string_append(out,"\\lyxformat 413\n");
	mmd_g_string_append(out, "\\begin_document\n");
	mmd_g_string_append(out, "\\begin_header\n");

	MMD_GString *lyx_class = mmd_g_string_new("");
	if (tree_contains_key(list, METAKEY)) {
		latex_mode = metadata_for_key("latexmode",list);
		if (latex_mode != NULL) {
			key = metavalue_for_key("latexmode",list);
			label = label_from_string(key);
			mmd_g_string_append(lyx_class,label);
			...
			mmd_g_string_append(lyx_class,"memoir");
		}

	}else{
	     mmd_g_string_append(lyx_class,"memoir");
	}
	mmd_g_string_append(out,"\\textclass ");
	mmd_g_string_append_printf(out,"%s",lyx_class->str);
	mmd_g_string_append(out,"\n");

	mmd_g_string_append(out,"\\begin_preamble\n");
	mmd_g_string_append(out,"\\usepackage{listings}\n");
	mmd_g_string_append(out,"\\usepackage{natbib}\n");
	mmd_g_string_append(out,"\\usepackage{nomencl}\n");
	mmd_g_string_append(out,"\\usepackage{booktabs}\n");
	mmd_g_string_append(out,"\\usepackage{refstyle}\n");
	mmd_g_string_append(out,"\\usepackage{varioref}\n");
	...
	if (tree_contains_key(list, METAKEY)) {
		...
		if (packages != NULL) {
			...
			token = strtok(tmp, s);
			 while( token != NULL )  {
               mmd_g_string_append_printf(out,"\\usepackage{%s}\n",clean_string(token));
               token = strtok(NULL, s);
             }
			free(key);
			free(tmp);
	...
	if(isbeamer){
	  if (tree_contains_key(list, METAKEY)) {
		...
		if (content != NULL) {
			...
			mmd_g_string_append_printf(out,"\\usetheme{%s}\n",value);
			free(value);
		} else{
			mmd_g_string_append(out,"\\usetheme{warsaw}\n");
		}
	  } else {
		mmd_g_string_append(out,"\\usetheme{warsaw}\n");
	  }
	  mmd_g_string_append(out,"\\setbeamercovered{transparent}\n");
	}
	...
	if (tree_contains_key(list, METAKEY)) {
		...
		if (content != NULL) {
			...
			if (strcmp(value,"mmd-natbib-plain")==0){
				mmd_g_string_append(out,"\\bibpunct{[}{]}{;}{n}{}{,}\n");
			}else{
				mmd_g_string_append(out,"\\bibpunct{(}{)}{,}{a}{,}{,}\n");
			}
			free(value);
		} else{
			mmd_g_string_append(out,"\\bibpunct{(}{)}{,}{a}{,}{,}\n");
	...
	mmd_g_string_append(out,"\\end_preamble\n");

	MMD_GString *class_options = mmd_g_string_new("\\options refpage");
	...
	if (tree_contains_key(list, METAKEY)) {
		...
		if (clean_pdf != NULL) {
			...
			label = label_from_string(key);
			if (strcmp(label, "yes") == 0) {
				mmd_g_string_append(class_options,",hidelinks");
			}
			free(label);
			free(key);
		}
    ...
    if (tree_contains_key(list, METAKEY)) {
		...
		if (content != NULL) {
			...
			mmd_g_string_append(class_options,",");
			mmd_g_string_append(class_options,value);
			free(value);
		}
	}
	mmd_g_string_append(class_options,"\n");
	mmd_g_string_append(out,class_options->str);
	mmd_g_string_free(class_options,TRUE);



	mmd_g_string_append(out,"\\begin_modules\n");
	if (tree_contains_key(list, METAKEY)) {
	 ...
	 mmd_g_string_append(out,"\\end_modules\n");

	mmd_g_string_append(out,"\\bibtex_command default\n");
	mmd_g_string_append(out,"\\cite_engine natbib_authoryear\n");

	mmd_g_string_free(lyx_class,TRUE);

	mmd_g_string_append(out,"\\end_header\n");
	mmd_g_string_append(out,"\\begin_body\n");
...
if (tree_contains_key(list, METAKEY)) {
	...
	mmd_g_string_append(out, "\n\\end_body\n");
	mmd_g_string_append(out, "\\end_document\n");
}
...
//			temp = ascii_label_from_string(n->children->str);
		    ...
		    mmd_g_string_append_printf(temp_str,"[%s]",width);
		    if(strstr(used_abbreviations->str,temp_str->str)){
		    	mmd_g_string_append(out,width); // just the abbrev
		    }
		    else
		    {
		    mmd_g_string_append(used_abbreviations,temp_str->str);


			mmd_g_string_append(out,n->children->str);
			mmd_g_string_append_printf(out," (%s)",width);
...
//            g_string_append(out,n->children->str);
            ...
            mmd_g_string_append(out,temp);
			mmd_g_string_append(out,"\"");
			mmd_g_string_append(out, "\n\\end_inset\n");
			free(temp);
            }
            mmd_g_string_free(temp_str,TRUE);
			free(width);
			break;
		case ABBRSTOP:
			break;
		case SPACE:
			if (strncmp(n->str,"\n",1)==0){
			  if (no_newline){
			  	mmd_g_string_append(out," "); /* just a space */
			  } else{
			    mmd_g_string_append_printf(out,"%s ",n->str); /* lyx needs the space */
		      }
			}else{
		...
		case HRULE:
			...
			mmd_g_string_append(out,"LatexCommand rule\n");
			mmd_g_string_append(out,"offset \"0.5ex\"\n");
			mmd_g_string_append(out,"width \"100col%\"\n");
			mmd_g_string_append(out,"height \"1pt\"\n");
			mmd_g_string_append(out,"\n\\end_inset\n");
			mmd_g_string_append(out,"\n\\end_layout\n");
			break;
		case HTMLBLOCK:
		...
		case VERBATIMFENCE:
			...
			mmd_g_string_append(out,"\\begin_layout Standard\n");
			mmd_g_string_append(out,"\\begin_inset listings\n");
			if ((n->children != NULL) && (n->children->key == VERBATIMTYPE)) {
				...
				if (strlen(n->children->str) > 0) {
					...
					mmd_g_string_append_printf(out, "lstparams \"basicstyle={\\footnotesize\\ttfamily},language=%s\"\n", n->children->str,n->str);
				}
			   else {
			   	 	mmd_g_string_append(out,"lstparams \"basicstyle={\\footnotesize\\ttfamily}\"\n");
			   }
			} else {
		 	    mmd_g_string_append(out,"lstparams \"basicstyle={\\footnotesize\\ttfamily}\"\n");
		       }
			mmd_g_string_append(out,"inline false\n");
			mmd_g_string_append(out,"status collapsed\n");
			print_lyx_string(out, n->str, scratch,LYX_PLAIN); /* it is not children - just \n separated lines */
			mmd_g_string_append(out,"\n\\end_inset\n");
			mmd_g_string_append(out,"\\end_layout\n");
			 scratch->lyx_level--;
		...
		case H1: case H2: case H3: case H4: case H5: case H6:
			...
			MMD_GString *environment = mmd_g_string_new("\n\\begin_layout ");
			mmd_g_string_append(environment,heading_name[lev-1]->str); /* get the (possibly user modified) section name */

				if (!scratch->lyx_number_headers){
				mmd_g_string_append(environment,"*\n");} /* mark as unnumbered */
				else{
				mmd_g_string_append(environment,"\n");
		     	};
			/* Begin the environment */
			    mmd_g_string_append_printf(out,"%s",environment->str);
			mmd_g_string_free(environment,true);
		...
		case MATHSPAN:
			if (n->str[0] == '$') {
				...
				if (n->str[strlen(n->str)-1] == ']') {
					n->str[strlen(n->str)-3] = '\0';
					mmd_g_string_append(out,"\\begin_inset Formula \n\\[");
					mmd_g_string_append_printf(out, "\n%s\n\\]\n\\end_inset\n", &n->str[2]);
				} else {
					n->str[strlen(n->str)-3] = '\0';
					mmd_g_string_append_printf(out, "\n\\begin_inset Formula $%s$\n\\end_inset\n", &n->str[2]);
	...
	fprintf(stderr, "have label for LyX link: '%s'\n",n->str);
				...
				if (n->link_data == NULL) {
					...
					if (n->children->next != NULL) {
						...
						mmd_g_string_append_printf(out, "[%s]",temp);
					}
					free(temp);
					break;
				}
			...
			if ((n->link_data->source != NULL) && (n->link_data->source[0] == '#' )) {
				...
				if (strlen(temp_str->str) > 0) {
					...
					mmd_g_string_append_printf(out, "%s (", temp_str->str);
				}
				...
				/* This is a <link> */
	            mmd_g_string_append_printf(out, "\n\\begin_inset CommandInset href\nLatexCommand href\ntarget \"%s\"\n", n->link_data->source);
				mmd_g_string_append_printf(out,"name \"%s\"",temp_str->str);
				mmd_g_string_append(out, "\n\n\\end_inset\n");
			...
			mmd_g_string_free(temp_str, TRUE);
			mmd_g_string_free(raw_str, true);
			n->link_data->attr = NULL;
			break;
		case ATTRKEY:
			mmd_g_string_append_printf(out, " %s=\"%s\"", n->str,
				n->children->str);
			break;
		case REFNAME:
		case SOURCE:
			...
			if (n->link_data->attr != NULL) {
				...
				if (width != NULL) {
					if (width[strlen(width)-1] == '%') {
						...
						mmd_g_string_append_printf(out, "\tlyxscale %s\n", width);
						mmd_g_string_append_printf(out, "\tscale %s\n",width);
					} else {
						mmd_g_string_append_printf(out, "\twidth %s\n",width);
					}
				}
				...
				if (height != NULL) {
					if (height[strlen(height)-1] == '%') {
						...
						mmd_g_string_append_printf(out, "\theight %s\n",height);
					}
				}
			mmd_g_string_append(out,"\n\\end_inset\n");
	...
	fprintf(stderr, "\nprint cite\n");
			...
			if ((n->link_data != NULL) && (strncmp(n->link_data->label,"[#",2) == 0)) {
				...
				if (n->key == NOCITATION) {
					...
					mmd_g_string_append(out, "<FAKE span class=\"externalcitation\">");
					mmd_g_string_append(out, "</span>");
				}
			} else {
#ifdef DEBUG_ON
				...
				fprintf(stderr, "cite without children. locat:'%s'\n",n->str);
							...
							if (strcmp(&temp[strlen(temp) - 1],";") == 0) {
								temp[strlen(temp) - 1] = '\0';
								mmd_g_string_append_printf(out, " \\citet{%s}",temp);
							} else {
								mmd_g_string_append(out,"\n\\begin_inset CommandInset citation");
							    mmd_g_string_append(out,"\nLatexCommand cite");
							    mmd_g_string_append_printf(out,"\nkey \"%s\"",temp);
...
//              g_string_append(out,"\n\n\\end_layout\n\\end_inset\n\\end_layout\n");
		    ...
		    /* replace spaces with protected spaces */
				...
				while (*temp != '\0'){
					if (*temp == ' '){
						...
						mmd_g_string_append_printf(out,"%c",*temp);
					}
					temp++;
				}
			mmd_g_string_free(temp_str,TRUE);
		...
		case TABLESEPARATOR:
			...
			for (i=0;i<scratch->lyx_table_total_cols;i++){
			  ...
			  switch(char_temp){
				case 'c':
				case 'C':
					mmd_g_string_append(temp_str,"center");
					break;
				case 'r':
				case 'R':
					mmd_g_string_append(temp_str,"right");
					break;
				case 'l':
				case 'L':
					mmd_g_string_append(temp_str,"left");
				    break;
		      }
		...
		case TABLECELL:
			...
			for(i=1;i<=multicol;i++){
	            ...
	            mmd_g_string_append(out,"\n<cell" ) ;
	            if (multicol > 1) {
					mmd_g_string_append_printf(out, " multicolumn=\"%d\"",i);
				}
				mmd_g_string_append_printf(out, " alignment=\"%s\"",temp_str->str);
				mmd_g_string_append(out, " valignment=\"top\"");

				if (scratch->lyx_table_need_line){
				   mmd_g_string_append(out," topline=\"true\"" );
			    }
			    if (scratch->table_row >= scratch->lyx_table_total_rows-1){
			       mmd_g_string_append(out," bottomline=\"true\"" );
			    }

			    mmd_g_string_append(out," usebox=\"none\"");
...
void print_lyx_localized_typography(MMD_GString *out, unsigned char character, scratch_pad *scratch) {
	if (!extension(EXT_SMART, scratch->extensions)) {
		mmd_g_string_append_c(out, character);
		return;
	}
	switch (character) {
		case LSQUOTE:
	...
	while (*str != '\0') {
		...
		if ((((unsigned char)*str & 0xc0) == 0xc0) && ((((unsigned char)*(str+1))  & 0xc0) == 0x80)) { /* hit unicode (huristic */
		   ...
		   while ((((unsigned char)*str != '\0')) && (((unsigned char)*str & 0xc0) == 0x80)){
		   	mmd_g_string_append_c(out,*str); /* send out the other bytes */
		   	str++;
		   }
		} else {
...
void print_lyx_url(MMD_GString *out, char *str, scratch_pad *scratch) {
	...
	while (*str != '\0') {
		switch (*str) {
			case '$': case '%': case '!':
			case '&': case '_': case '#':
				mmd_g_string_append_printf(out, "\\%c", *str);
				break;
			case '^':
				mmd_g_string_append(out, "\\^{}");
				break;
	...
	while (list != NULL) {
		switch (list->key) {
			case H1: case H2: case H3: case H4: case H5: case H6:
				...
				pound_label = mmd_g_string_new("#");
                mmd_g_string_append(pound_label,label);
				...
				mmd_g_string_free(pound_label,TRUE);
				free(label);
				break;
			case TABLE:
...
char *prefix_label(char *prefix, char *label, bool pound) {
	...
	MMD_GString *result = mmd_g_string_new("");
	if (label[0]== '#'){
		mmd_g_string_append(result,label+1);  /* drop the pound */
	} else{
		mmd_g_string_append(result,label);
	}
	mmd_g_string_prepend(result,":"); /* prefix colon */
	mmd_g_string_prepend(result,short_prefix); /* add the prefix */
	if (pound){
		mmd_g_string_prepend(result,"#"); /* add back in the pound if needed */
	}
	function_result = result->str;
	mmd_g_string_free(result,FALSE); /* sending back the string */
