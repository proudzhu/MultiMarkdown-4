...
void begin_odf_output(MMD_GString *out, node* list, scratch_pad *scratch) {
	...
	if (list == NULL) {
		mmd_g_string_append_printf(out, "<office:body>\n<office:text>\n");
	}
}
...
void print_odf_node(MMD_GString *out, node *n, scratch_pad *scratch) {
	...
	switch (n->key) {
		...
		case PARA:
			pad(out, 2, scratch);
			mmd_g_string_append_printf(out, "<text:p");
			switch (scratch->odf_para_type) {
				case DEFINITION:
				case BLOCKQUOTE:
					mmd_g_string_append_printf(out, " text:style-name=\"Quotations\"");
					break;
				case CODE:
				case VERBATIM:
				case VERBATIMFENCE:
					mmd_g_string_append_printf(out, " text:style-name=\"Preformatted Text\"");
					break;
				case ORDEREDLIST:
					mmd_g_string_append_printf(out, " text:style-name=\"P2\"");
					break;
				case BULLETLIST:
					mmd_g_string_append_printf(out, " text:style-name=\"P1\"");
					break;
				case NOTEREFERENCE:
				case NOTESOURCE:
				case CITATION:
				case NOCITATION:
					mmd_g_string_append_printf(out, " text:style-name=\"Footnote\"");
					break;
				default:
					mmd_g_string_append_printf(out, " text:style-name=\"Standard\"");
					break;
			}
		...
		case HRULE:
			pad(out, 2, scratch);
			mmd_g_string_append_printf(out,"<text:p text:style-name=\"Horizontal_20_Line\"/>");
			scratch->padded = 0;
			break;
		case HTMLBLOCK:
			...
			if (strncmp(n->str,"<!--",4) == 0) {
				...
				mmd_g_string_append_printf(out, "<text:p text:style-name=\"Standard\">%s</text:p>", &n->str[4]);
				scratch->padded = 0;
			}
			break;
		case VERBATIM:
		case VERBATIMFENCE:
			...
			pad(out, 2, scratch);
			mmd_g_string_append_printf(out, "<text:p text:style-name=\"Preformatted Text\">");
		...
		case ORDEREDLIST:
			...
			switch (n->key) {
				case BULLETLIST:
					mmd_g_string_append_printf(out, "<text:list text:style-name=\"L1\">");
					break;
				case ORDEREDLIST:
					mmd_g_string_append_printf(out, "<text:list text:style-name=\"L2\">");
					break;
			}
			scratch->padded = 1;
			print_odf_node_tree(out, n->children, scratch);
			pad(out, 1, scratch);
			mmd_g_string_append_printf(out, "</text:list>");
			scratch->padded = 0;
			scratch->odf_para_type = old_type;
			break;
		case LISTITEM:
...
#endif
			pad(out, 1, scratch);
			mmd_g_string_append_printf(out, "<text:list-item>\n");
			if ((n->children != NULL) && (n->children->children != NULL) && (n->children->children->key != PARA)) {
				switch (scratch->odf_para_type) {
					case BULLETLIST:
						mmd_g_string_append_printf(out, "<text:p text:style-name=\"P1\">");
						break;
					case ORDEREDLIST:
						mmd_g_string_append_printf(out, "<text:p text:style-name=\"P2\">");
						break;
				}
	...
	fprintf(stderr, "finish print list item\n");
			...
			break;
		case METADATA:
			mmd_g_string_append_printf(out, "<office:meta>\n");
			scratch->extensions = scratch->extensions | EXT_HEAD_CLOSED;
			print_odf_node_tree(out, n->children, scratch);
			mmd_g_string_append_printf(out, "</office:meta>\n");
		...
		case METAKEY:
			...
			if (strcmp(temp, "title") == 0) {
				mmd_g_string_append_printf(out, "<dc:title>");
				...
				mmd_g_string_append_printf(out, "<meta:keyword>");
				print_odf_node(out, n->children, scratch);
				mmd_g_string_append_printf(out, "</meta:keyword>\n");
				...
				mmd_g_string_append_printf(out,"<meta:user-defined meta:name=\"");
				print_odf_string(out, n->str);
				mmd_g_string_append_printf(out, "\">");
				print_odf_node(out,n->children,scratch);
				mmd_g_string_append_printf(out,"</meta:user-defined>\n");
			}
			free(temp);
			break;
		case METAVALUE:
		...
		case H1: case H2: case H3: case H4: case H5: case H6:
			...
			pad(out, 2, scratch);
			mmd_g_string_append_printf(out, "<text:h text:outline-level=\"%d\">", lev);
			if (n->children->key == AUTOLABEL) {
				/* use label for header since one was specified (MMD)*/
				temp = label_from_string(n->children->str);
				mmd_g_string_append_printf(out, "<text:bookmark text:name=\"%s\"/>", temp);
				print_odf_node_tree(out, n->children->next, scratch);
				mmd_g_string_append_printf(out, "<text:bookmark-end text:name=\"%s\"/>", temp);
				free(temp);
			} else {
			...
			mmd_g_string_append_printf(out, "</text:h>");
			scratch->padded = 0;
			break;
		case APOSTROPHE:
		...
		case DOUBLEQUOTED:
			...
			break;
		case LINEBREAK:
			mmd_g_string_append_printf(out, "<text:line-break/>");
			break;
		case MATHSPAN:
			temp = strdup(n->str);
			if (temp[0] == '$') {
				mmd_g_string_append_printf(out, "<text:span text:style-name=\"math\">%s</text:span>",temp);
			...
			break;
		case STRONG:
			mmd_g_string_append_printf(out, "<text:span text:style-name=\"MMD-Bold\">");
			print_odf_node_tree(out,n->children,scratch);
			mmd_g_string_append_printf(out, "</text:span>");
			break;
		case EMPH:
			mmd_g_string_append_printf(out, "<text:span text:style-name=\"MMD-Italic\">");
			print_odf_node_tree(out,n->children,scratch);
			mmd_g_string_append_printf(out, "</text:span>");
			break;
		case LINKREFERENCE:
	...
	fprintf(stderr, "got link data for odf link: '%s'\n",n->str);
#endif
			mmd_g_string_append_printf(out, "<text:a xlink:type=\"simple\"");
			if (n->link_data->source != NULL) {
				mmd_g_string_append_printf(out, " xlink:href=\"");
				print_html_string(out,n->link_data->source, scratch);
				mmd_g_string_append_printf(out, "\"");
			}
			if ((n->link_data->title != NULL) && (strlen(n->link_data->title) > 0)) {
				mmd_g_string_append_printf(out, " office:name=\"");
			...
			if (n->children != NULL)
				print_odf_node_tree(out,n->children,scratch);
			mmd_g_string_append_printf(out, "</text:a>");
	...
	fprintf(stderr, "create img\n");
#endif
			mmd_g_string_append_printf(out, "<draw:frame text:anchor-type=\"as-char\"\ndraw:z-index=\"0\" draw:style-name=\"fr1\" ");
			...
			if (width != NULL) {
				mmd_g_string_append_printf(out, "svg:width=\"%s\"\n", width);
			} else {
				mmd_g_string_append_printf(out, "svg:width=\"95%%\"\n");
			}

			mmd_g_string_append_printf(out, ">\n<draw:text-box><text:p><draw:frame text:anchor-type=\"as-char\" draw:z-index=\"1\" ");
			if ((height != NULL) && (width != NULL)) {
				mmd_g_string_append_printf(out, "svg:height=\"%s\"\n",height);
				mmd_g_string_append_printf(out, "svg:width=\"%s\"\n", width);
			}

			if (n->link_data->source != NULL)
				mmd_g_string_append_printf(out, "><draw:image xlink:href=\"%s\"",n->link_data->source);

			mmd_g_string_append_printf(out," xlink:type=\"simple\" xlink:show=\"embed\" xlink:actuate=\"onLoad\" draw:filter-name=\"&lt;All formats&gt;\"/>\n</draw:frame></text:p>");
			...
			if (n->key == IMAGEBLOCK) {
				if (n->children != NULL) {
					...
					mmd_g_string_append_printf(out, "<text:p>Figure <text:sequence text:name=\"Figure\" text:formula=\"ooow:Figure+1\" style:num-format=\"1\"> Update Fields to calculate numbers</text:sequence>: ");
						mmd_g_string_append(out, temp_str->str);
						mmd_g_string_append_printf(out, "</text:p>");
					}
					mmd_g_string_free(temp_str, true);
				}
				mmd_g_string_append_printf(out, "</draw:text-box></draw:frame>\n</text:p>\n");
			} else {
				mmd_g_string_append_printf(out, "</draw:text-box></draw:frame>\n");
			}
			scratch->padded = 1;
	...
	fprintf(stderr, "finish image\n");
		...
		case NOTEREFERENCE:
			...
			if (temp_node->key == GLOSSARYSOURCE) {
				mmd_g_string_append_printf(out, "<text:note text:id=\"\" text:note-class=\"glossary\"><text:note-body>\n");
				print_odf_node_tree(out, temp_node->children, scratch);
				mmd_g_string_append_printf(out, "</text:note-body>\n</text:note>\n");
			} else {
				mmd_g_string_append_printf(out, "<text:note text:id=\"\" text:note-class=\"footnote\"><text:note-body>\n");
				print_odf_node_tree(out, temp_node->children, scratch);
				mmd_g_string_append_printf(out, "</text:note-body>\n</text:note>\n");
			}
	...
	fprintf(stderr, "print cite\n");
			...
			if ((n->link_data != NULL) && (strncmp(n->link_data->label,"[#",2) == 0)) {
				...
				if (lev != 0) {
					...
					if (lev > scratch->max_footnote_num) {
						...
						mmd_g_string_append_printf(out, "<text:note text:id=\"cite%d\" text:note-class=\"endnote\"><text:note-body>\n", lev);
						scratch->padded = 2;
					...
					fprintf(stderr, "renumbered to %d\n", lev);
						...
						mmd_g_string_append_printf(out, "<text:span text:style-name=\"Footnote_20_anchor\"><text:note-ref text:note-class=\"endnote\" text:reference-format=\"text\" text:ref-name=\"cite%d\">%d</text:note-ref></text:span>", lev, lev);
					}
				} else {
		...
		case VARIABLE:
			...
			break;
		case GLOSSARYTERM:
			mmd_g_string_append_printf(out,"<text:p text:style-name=\"Glossary\">");
		...
		case GLOSSARYSORTKEY:
			break;
		case CODE:
			mmd_g_string_append_printf(out, "<text:span text:style-name=\"Source_20_Text\">");
			print_html_string(out, n->str, scratch);
			mmd_g_string_append_printf(out, "</text:span>");
			break;
		...
		case HTML:
			...
			if (strncmp(n->str,"<!--",4) == 0) {
				/* trim "-->" from end */
				n->str[strlen(n->str)-3] = '\0';
				mmd_g_string_append_printf(out, "%s", &n->str[4]);
			}
			break;
		case DEFLIST:
			print_odf_node_tree(out, n->children, scratch);
		...
		case TERM:
			pad(out,1, scratch);
			mmd_g_string_append_printf(out, "<text:p><text:span text:style-name=\"MMD-Bold\">");
			print_odf_node_tree(out, n->children, scratch);
			mmd_g_string_append_printf(out, "</text:span></text:p>\n");
			scratch->padded = 1;
			break;
		case DEFINITION:
			old_type = scratch->odf_para_type;
			...
			pad(out,1, scratch);
			scratch->padded = 1;
			mmd_g_string_append_printf(out, "<text:p text:style-name=\"Quotations\">");
			print_odf_node_tree(out, n->children, scratch);
			mmd_g_string_append_printf(out, "</text:p>\n");
			scratch->padded = 0;
			scratch->odf_para_type = old_type;
		...
		case TABLE:
			pad(out,2, scratch);
			mmd_g_string_append_printf(out, "<table:table>\n");
			print_odf_node_tree(out, n->children, scratch);
			mmd_g_string_append_printf(out, "</table:table>\n");
			/* caption if present */
			if ((n->children != NULL) && (n->children->key == TABLECAPTION)) {
				...
				mmd_g_string_append_printf(out,"<text:p><text:bookmark text:name=\"%s\"/>Table <text:sequence text:name=\"Table\" text:formula=\"ooow:Table+1\" style:num-format=\"1\"> Update Fields to calculate numbers</text:sequence>:", temp);
				print_odf_node_tree(out,n->children->children, scratch);
				mmd_g_string_append_printf(out, "<text:bookmark-end text:name=\"%s\"/></text:p>\n",temp);
				free(temp);
			}
			scratch->padded = 1;
		...
		case TABLEHEAD:
			for (i=0; i < strlen(scratch->table_alignment); i++) {
				mmd_g_string_append_printf(out, "<table:table-column/>\n");
			}
			scratch->cell_type = 'h';
			print_odf_node_tree(out, n->children, scratch);
			scratch->cell_type = 'd';
		...
		case TABLEBODY:
			...
			break;
		case TABLEROW:
			mmd_g_string_append_printf(out, "<table:table-row>\n");
		...
		case TABLECELL:
			...
			mmd_g_string_append_printf(out, "<table:table-cell");
			if ((n->children != NULL) && (n->children->key == CELLSPAN)) {
				mmd_g_string_append_printf(out, " table:number-columns-spanned=\"%d\"", strlen(n->children->str)+1);
				scratch->table_column += (int)strlen(n->children->str);
			}
			mmd_g_string_append_printf(out, ">\n<text:p");
			if (scratch->cell_type == 'h') {
				mmd_g_string_append_printf(out, " text:style-name=\"Table_20_Heading\"");
			} else {
				if ( strncmp(&temp[lev],"r",1) == 0) {
					mmd_g_string_append_printf(out, " text:style-name=\"MMD-Table-Right\"");
				} else if ( strncmp(&temp[lev],"R",1) == 0) {
					mmd_g_string_append_printf(out, " text:style-name=\"MMD-Table-Right\"");
				} else if ( strncmp(&temp[lev],"c",1) == 0) {
					mmd_g_string_append_printf(out, " text:style-name=\"MMD-Table-Center\"");
				} else if ( strncmp(&temp[lev],"C",1) == 0) {
					mmd_g_string_append_printf(out, " text:style-name=\"MMD-Table-Center\"");
				} else {
					mmd_g_string_append_printf(out, " text:style-name=\"MMD-Table\"");
				}
			}
		...
		case GLOSSARYLABEL:
			break;
		case SUPERSCRIPT:
			mmd_g_string_append_printf(out, "<text:span text:style-name=\"MMD-Superscript\">");
			print_html_string(out,n->str, scratch);
			mmd_g_string_append_printf(out, "</text:span>");
			break;
		case SUBSCRIPT:
			mmd_g_string_append_printf(out, "<text:span text:style-name=\"MMD-Subscript\">");
			print_html_string(out,n->str, scratch);
			mmd_g_string_append_printf(out, "</text:span>");
			break;
		case KEY_COUNTER:
			...
			case ' ':
				...
				if (*tmp == ' ') {
					...
					if (*tmp == ' ') {
						tmp++;
						if (*tmp == ' ') {
							mmd_g_string_append_printf(out, "<text:tab/>");
							str = tmp;
						} else {
							mmd_g_string_append_printf(out, " ");
						}
...
"     office:mimetype=\"application/vnd.oasis.opendocument.text\">\n");
	...
	/* Font Declarations */
	mmd_g_string_append_printf(out, "<office:font-face-decls>\n" \
    ...
    mmd_g_string_append_printf(out, "<office:styles>\n" \
    ...
    mmd_g_string_append_printf(out, "<office:automatic-styles>" \
...
void print_odf_footer(MMD_GString *out) {
    mmd_g_string_append_printf(out, "</office:text>\n</office:body>\n</office:document>");
}
